...
## version: "3.14.0" — Convoke the Spirits replaced by Incarnation, 2026-09-08

Talent build changed from Convoke the Spirits + Carnivorous Instinct + Rip and
Tear to **Incarnation: Avatar of Ashamane + Ashamane's Guidance + Hunger for
Battle** — the rest of the Druid of the Claw hero tree and every other class
talent is identical. This is the community-reported most-used M+ high-key
build at the time of the change (checked against a maxroll.gg talent export),
not a guess.

Measured with the SAME priority (`sim/apl_feral/fixed_all.simc`, generated by
`ab_test_feral.py`'s `fixed_all` variant) against both talent strings, gear
and everything else held constant — only `talents=` changed on the SimC
command line. Fight style DungeonSlice, `sim/Ferraz_feral.simc` gear,
`target_error=0.2`:

    Convoke (previous):     130,334 DPS +/- 133
    Incarnation (new):      133,714 DPS +/- 136
    delta: +2.59%, combined error ~380 — real, not noise.

Raw output: `sim/out_feral/convoke.json`, `sim/out_feral/incarnation.json`.

**Why the priority needed no APL changes to pick this up:** `var.bs_inc`
(`buff.berserk.up|buff.incarnation_avatar_of_ashamane.up`) already treated
Berserk and Incarnation as the same button — this file has supported either
talent choice since it was written. The `berserk` action line just casts
whichever one is talented.

**What DID need to change:** the `convoke_the_spirits` line in `cooldown`
became dead (the talent no longer exists on this string) and was removed
outright rather than left as inert weight. `holdBerserk` existed to sync
Berserk's cooldown with Convoke's so both burst windows lined up — with no
Convoke to sync against, it is pinned to `0` rather than left evaluating
`cooldown.convoke_the_spirits.remains` on an untalented spell, which is
exactly the "condition that can never turn false/true predictably" bug class
this file has hit before on unrelated lines. `ttd_convoke` (config slider)
and `convoke_ttd_ok` (its TTD variable) were removed for the same reason —
nothing left to gate.

**Not investigated:** Ashamane's Guidance's actual mechanic has no dedicated
APL line, on the same reasoning as Unseen Predator elsewhere in this file —
if it turns out to need one (extending Incarnation, a proc, anything active),
that is a separate, unmeasured gain on top of the +2.59% above.
